...
//
// Bootup the masternode, look for a 7280 STARS input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* rewards are not supported in StarCashNetwork.conf */
...
{
    CStarCashNetworkcoinAddress address(collateralAddress);
