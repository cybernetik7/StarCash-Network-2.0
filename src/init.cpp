...
void Shutdown()
{
    ...
    RenameThread("StarCashNetwork-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: StarCashNetwork.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: StarCashNetworkd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeStarCashNetworkamount=<n> " + _("Keep N StarCashNetwork anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. StarCashNetwork is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. StarCashNetwork is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("StarCashNetwork version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "StarCashNetwork server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of StarCashNetwork") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart StarCashNetwork to complete") << "\n";
    ...
    nAnonymizeStarCashNetworkAmount = GetArg("-anonymizeStarCashNetworkamount", 0);
    if(nAnonymizeStarCashNetworkAmount > 999999) nAnonymizeStarCashNetworkAmount = 999999;
    if(nAnonymizeStarCashNetworkAmount < 2) nAnonymizeStarCashNetworkAmount = 2;
    ...
    LogPrintf("Anonymize StarCashNetwork Amount %d\n", nAnonymizeStarCashNetworkAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1STARS+1000 == (.1STARS+100)*10
       10STARS+10000 == (1STARS+1000)*10
    */
