...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("StarCashNetwork"))
        return false;
...
{
    // Convert StarCashNetwork:// to StarCashNetwork:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("StarCashNetwork://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "StarCashNetwork:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "StarCashNetwork.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for StarCashNetwork.lnk
...
{
    return GetAutostartDir() / "StarCashNetwork.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a StarCashNetwork.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=StarCashNetwork\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the StarCashNetwork app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add StarCashNetwork app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("StarCashNetwork-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  StarCashNetwork-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("StarCashNetwork-Qt"));
