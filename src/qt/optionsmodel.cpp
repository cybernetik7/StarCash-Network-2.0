...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeStarCashNetworkAmount"))
        settings.setValue("nAnonymizeStarCashNetworkAmount", 1000);
    nAnonymizeStarCashNetworkAmount = settings.value("nAnonymizeStarCashNetworkAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeStarCashNetworkAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeStarCashNetworkAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeStarCashNetworkAmount:
            return QVariant(nAnonymizeStarCashNetworkAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeStarCashNetworkAmount:
            nAnonymizeStarCashNetworkAmount = value.toInt();
            settings.setValue("nAnonymizeStarCashNetworkAmount", nAnonymizeStarCashNetworkAmount);
            emit AnonymizeStarCashNetworkAmountChanged(nAnonymizeStarCashNetworkAmount);
            break;
        default:
            break;
        }
