...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeStarCashNetworkAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeStarCashNetworkAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeStarCashNetworkAmount = strAnonymizeStarCashNetworkAmount.remove(strAnonymizeStarCashNetworkAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeStarCashNetworkAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeStarCashNetworkAmount*COIN) nMaxToAnonymize = nAnonymizeStarCashNetworkAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeStarCashNetworkAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeStarCashNetworkAmount));
        strAnonymizeStarCashNetworkAmount = strAnonymizeStarCashNetworkAmount.remove(strAnonymizeStarCashNetworkAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeStarCashNetworkAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeStarCashNetworkAmount == 0){
