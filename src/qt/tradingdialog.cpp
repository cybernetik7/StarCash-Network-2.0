    ...
    model(0)
{
    ...
    ui->STARSAvailableLabel->setTextFormat(Qt::RichText);
    ...
    ui->BittrexSTARSLabel->setTextFormat(Qt::RichText);
    ...
    CreateOrderBookTables(*ui->BidsTable,QStringList() << "SUM(BTC)" << "TOTAL(BTC)" << "STARS(SIZE)" << "BID(BTC)");
    CreateOrderBookTables(*ui->AsksTable,QStringList() << "ASK(BTC)" << "STARS(SIZE)" << "TOTAL(BTC)" << "SUM(BTC)");
    /*OrderBook Table Init*/

    /*Market History Table Init*/
    ui->MarketHistoryTable->setColumnCount(5);
    ui->MarketHistoryTable->verticalHeader()->setVisible(false);
    ui->MarketHistoryTable->setHorizontalHeaderLabels(QStringList()<<"DATE"<<"BUY/SELL"<<"BID/ASK"<<"TOTAL UNITS(STARS)"<<"TOTAL COST(BTC");
...
void tradingDialog::UpdaterFunction(){
    //STARSst get the main exchange info in order to populate qLabels in maindialog. then get data
...
QString tradingDialog::GetMarketSummary(){
     ...
     QString Response = sendRequest("https://bittrex.com/api/v1.1/public/GetMarketSummary?market=btc-STARS");
     return Response;
}

QString tradingDialog::GetOrderBook(){

      QString  Response = sendRequest("https://bittrex.com/api/v1.1/public/getorderbook?market=BTC-STARS&type=both&depth=50");
      return Response;
}

QString tradingDialog::GetMarketHistory(){
      QString Response = sendRequest("https://bittrex.com/api/v1.1/public/getmarkethistory?market=BTC-STARS&count=100");
      return Response;
}
...
QString tradingDialog::BuySTARS(QString OrderType, double Quantity, double Rate){
    ...
    QString URL = "https://bittrex.com/api/v1.1/market/";
            ...
            URL += "?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-STARS&quantity=";
...
QString tradingDialog::SellSTARS(QString OrderType, double Quantity, double Rate){
...
QString tradingDialog::GetOpenOrders(){
    QString URL = "https://bittrex.com/api/v1.1/market/getopenorders?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-STARS";
...
QString tradingDialog::GetDepositAddress(){
    ...
    QString URL = "https://bittrex.com/api/v1.1/account/getdepositaddress?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&currency=STARS";

    QString Response = sendRequest(URL);
    return Response;
}

QString tradingDialog::GetSTARSuntHistory(){

    QString URL = "https://bittrex.com/api/v1.1/account/getorderhistory?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-STARS&count=10";
...
int tradingDialog::SetExchangeInfoTextLabels(){
    ...
    ui->volumet->setText("<b>STARS Volume:</b> <span style='font-weight:bold; font-size:12px; color:blue;'>" + str.number(obj["Volume"].toDouble(),'i',8) + "</span> STARS");
...
void tradingDialog::ParseAndPopulateSTARSuntHistoryTable(QString Response){
...
void tradingDialog::ParseAndPopulateOrderBookTables(QString OrderBook){
    ...
    double STARSSupply = 0;
    double STARSDemand = 0;
    double BtcSupply = 0;
    double BtcDemand = 0;
    ...
    ui->STARSSupply->setText("<b>Supply:</b> <span style='font-weight:bold; font-size:12px; color:blue'>" + str.number(STARSSupply,'i',8) + "</span><b> STARS</b>");
    ...
    ui->STARSDemand->setText("<b>Demand:</b> <span style='font-weight:bold; font-size:12px; color:blue'>" + str.number(STARSDemand,'i',8) + "</span><b> STARS</b>");
...
void tradingDialog::ActionsOnSwitch(int index = -1){
    ...
    switch (index){
                case 0:    //buy tab is active
                    ...
                    Response2 = GetBalance("STARS");
                    Response3 = GetOrderBook();

                    if((Response.size() > 0 && Response != "Error") && (Response2.size() > 0 && Response2 != "Error")){
                        DisplayBalance(*ui->BtcAvailableLabel, *ui->STARSAvailableLabel, Response, Response2);
                    }
                ...
                case 1: //Cross send tab active
                    Response = GetBalance("STARS");
                    Response2 = GetBalance("BTC");
                    if((Response.size() > 0 && Response != "Error") && (Response2.size() > 0 && Response2 != "Error")){
                        DisplayBalance(*ui->BittrexSTARSLabel, *ui->BittrexBTCLabel, Response, Response2);
                    }
                ...
                case 4://account history tab
                    Response = GetSTARSuntHistory();
                    if(Response.size() > 0 && Response != "Error"){
                        ParseAndPopulateSTARSuntHistoryTable(Response);
                    }
                break;
                ...
                case 5://show balance tab
                    ...
                    Response = GetBalance("STARS");
                    if(Response.size() > 0 && Response != "Error"){
                        DisplayBalance(*ui->STARSBalanceLabel,*ui->STARSAvailableLabel_2,*ui->STARSPendingLabel, QString::fromUtf8("STARS"),Response);
                    }
                break;
...
void tradingDialog::CalculateSellCostLabel(){
    ...
    double Quantity = ui->UnitsInputSTARS->text().toDouble();
    double cost = ((price * Quantity) - ((price * Quantity / 100) * 0.25));
...
void tradingDialog::CalculateCSReceiveLabel(){
    ...
    QString balance = GetBalance("STARS");
    ...
    double AvailableSTARS = BalanceObject["Available"].toDouble();
...
void tradingDialog::on_UpdateKeys_clicked(bool Save, bool Load)
{
  ...
  this->SecretKey = ui->SecretKeyInput->text();
   ...
   QJsonDocument jsonResponse = QJsonDocument::fromJson(GetSTARSuntHistory().toUtf8()); //get json from str.
...
{
    //calculate amount of BTC that can be gained from selling STARS available balance
    QString responseA = GetBalance("STARS");
    QString str;
    QJsonObject ResultObject =  GetResultObjectFromJSONObject(responseA);

    double AvailableSTARS = ResultObject["Available"].toDouble();

    ui->UnitsInputSTARS->setText(str.number(AvailableSTARS,'i',8));
}
...
{
    double Quantity = ui->BittrexSTARSLabel->text().toDouble();
    double Received = 0;
    double Qty = 0;
    double Price = 0;
    QString buyorders = GetOrderBook();
...
void tradingDialog::on_Withdraw_Max_Amount_clicked()
{
    ...
    ui->WithdrawUnitsInput->setText(str.number(AvailableSTARS,'i',8));
}
...
void tradingDialog::on_BuySTARS_clicked()
{
    double Rate;
    double Quantity;
    ...
    if (reply == QMessageBox::Yes) {
        ...
        QString Response =  BuySTARS(Order,Quantity,Rate);
...
void tradingDialog::on_SellSTARSBTN_clicked()
{
    double Rate;
    double Quantity;

    Rate     = ui->SellBidPriceEdit->text().toDouble();
    Quantity = ui->UnitsInputSTARS->text().toDouble();
    ...
    if (reply == QMessageBox::Yes) {
        ...
        QString Response =  SellSTARS(Order,Quantity,Rate);
        QJsonDocument jsonResponse = QJsonDocument::fromJson(Response.toUtf8());          //get json from str.
        QJsonObject  ResponseObject = jsonResponse.object();                              //get json obj
    ...
    if(!ctx.isValid())
    {
        ...
        foreach (const QJsonValue & value, BuyArray)
        {
            ...
            if ( ((Quantity / x) - y) > 0 )
            {
                ...
                QString SellResponse = SellSTARS(Order,y,x);
                ...
                QString SellResponse = SellSTARS(Order,(Quantity / x),x);
                QJsonDocument SelljsonResponse = QJsonDocument::fromJson(SellResponse.toUtf8());          //get json from str.
                QJsonObject SellResponseObject = SelljsonResponse.object();                              //get json obj
                ...
                if (SellResponseObject["success"].toBool() == false){
                    ...
                    if (ResponseObject["success"].toBool() == false){
                        ...
                        if (ResponseObject["success"].toBool() == false){
                            ...
                            QMessageBox::information(this,"Success","<center>Cross-Send Successful</center>\n Sold "+Astr.number(Qty,'i',4)+" STARS for "+Qstr.number((ui->CSUnitsInput->text().toDouble()-0.0002),'i',8)+" BTC");
                        }
                    } else if (ResponseObject["success"].toBool() == true){
                        QMessageBox::information(this,"Success","<center>Cross-Send Successful</center>\n Sold "+Astr.number(Qty,'i',4)+" STARS for "+Qstr.number((ui->CSUnitsInput->text().toDouble()-0.0002),'i',8)+" BTC");
                    }
...
void tradingDialog::on_UnitsInputSTARS_textChanged(const QString &arg1)
{
     CalculateSellCostLabel(); //update cost
}
