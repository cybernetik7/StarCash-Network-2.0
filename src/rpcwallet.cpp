...
static void accountingDeprecationCheck()
{
    if (!GetBoolArg("-enableaccounts", false))
        throw runtime_error(
            "STARSunting API is deprecated and will be removed in future.\n"
...
string STARSuntFromValue(const Value& value)
{
    string strSTARSunt = value.get_str();
    if (strSTARSunt == "*")
...
CScript _createmultisig(const Array& params)
{
    ...
    for (unsigned int i = 0; i < keys.size(); i++)
    {
        ...
        CStarCashNetworkcoinAddress address(ks);
...
Value createmultisig(const Array& params, bool fHelp)
{
    ...
    CStarCashNetworkcoinAddress address(innerID);
...
Value getnewpubkey(const Array& params, bool fHelp)
{
    ...
    string strSTARSunt;
    if (params.size() > 0)
        strSTARSunt = STARSuntFromValue(params[0]);
    ...
    pwalletMain->SetAddressBookName(keyID, strSTARSunt);
...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "\nReturns a new StarCashNetwork address for receiving payments.\n"
            "If 'account' is specified (recommended), it is added to the address book \n"
            "so payments received with the address will be credited to 'account'.\n"
            "\nArguments:\n"
            "1. \"account\"        (string, optional) The account name for the address to be linked to. if not provided, the default account \"\" is used. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created if there is no account by the given name.\n"
            "\nResult:\n"
            "\"StarCashNetworkaddress\"    (string) The new StarCashNetwork address\n"
            "\nExamples:\n"
    ...
    return CStarCashNetworkcoinAddress(keyID).ToString();
}


CStarCashNetworkcoinAddress GetSTARSuntAddress(string strSTARSunt, bool bForceNew=false)
{
    CWalletDB walletdb(pwalletMain->strWalletFile);

    CSTARSunt account;
    walletdb.ReadSTARSunt(strSTARSunt, account);
    ...
    if (!account.vchPubKey.IsValid() || bForceNew || bKeyUsed)
    {
        ...
        pwalletMain->SetAddressBookName(account.vchPubKey.GetID(), strSTARSunt);
        walletdb.WriteSTARSunt(strSTARSunt, account);
    }

    return CStarCashNetworkcoinAddress(account.vchPubKey.GetID());
}
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nReturns the current StarCashNetwork address for receiving payments to this account.\n"
            "\nArguments:\n"
            "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"
            "\nResult:\n"
            "\"StarCashNetworkaddress\"   (string) The account StarCashNetwork address\n"
            "\nExamples:\n"
    ...
    // Parse the account first so we don't generate a key if there's an error
    string strSTARSunt = STARSuntFromValue(params[0]);

    Value ret;

    ret = GetSTARSuntAddress(strSTARSunt).ToString();
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount \"StarCashNetworkaddress\" \"account\"\n"
            "\nSets the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"StarCashNetworkaddress\"  (string, required) The StarCashNetwork address to be associated with an account.\n"
    ...
    CStarCashNetworkcoinAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid StarCashNetwork address");


    string strSTARSunt;
    if (params.size() > 1)
        strSTARSunt = STARSuntFromValue(params[1]);
    ...
    if (IsMine(*pwalletMain, address.Get()))
    {
        ...
        {
            string strOldSTARSunt = pwalletMain->mapAddressBook[address.Get()];
            if (address == GetSTARSuntAddress(strOldSTARSunt))
                GetSTARSuntAddress(strOldSTARSunt, true);
        }
        pwalletMain->SetAddressBookName(address.Get(), strSTARSunt);
    }
    else
...
Value getaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "getaccount \"StarCashNetworkaddress\"\n"
            "\nReturns the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"StarCashNetworkaddress\"  (string, required) The StarCashNetwork address for account lookup.\n"
            "\nResult:\n"
...
Value getaddressesbyaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "  \"StarCashNetworkaddress\"  (string) a StarCashNetwork address associated with the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n"
    ...
    Array ret;
    BOOST_FOREACH(const PAIRTYPE(CStarCashNetworkcoinAddress, string)& item, pwalletMain->mapAddressBook)
    {
        const CStarCashNetworkcoinAddress& address = item.first;
        const string& strName = item.second;
        if (strName == strSTARSunt)
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "sendtoaddress \"StarCashNetworkaddress\" amount ( \"comment\" \"comment-to\" )\n"
            "\nSent an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
           "1. \"StarCashNetworkaddress\"  (string, required) The StarCashNetwork address to send to.\n"
            "2. \"amount\"      (numeric, required) The amount in STARS to send. eg 0.1\n"
...
Value listaddressgroupings(const Array& params, bool fHelp)
{
    if (fHelp)
        throw runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"StarCashNetworkaddress\",     (string) The StarCashNetwork address\n"
    ...
    BOOST_FOREACH(set<CTxDestination> grouping, pwalletMain->GetAddressGroupings())
    {
        ...
        {
            Array addressInfo;
            addressInfo.push_back(CStarCashNetworkcoinAddress(address).ToString());
            addressInfo.push_back(ValueFromAmount(balances[address]));
            {
                LOCK(pwalletMain->cs_wallet);
                if (pwalletMain->mapAddressBook.find(CStarCashNetworkcoinAddress(address).Get()) != pwalletMain->mapAddressBook.end())
                    addressInfo.push_back(pwalletMain->mapAddressBook.find(CStarCashNetworkcoinAddress(address).Get())->second);
            }
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage \"StarCashNetworkaddress\" \"message\"\n"
            "\nSign a message with the private key of an address"
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"StarCashNetworkaddress\"  (string, required) The StarCashNetwork address to use for the private key.\n"
    ...
    CStarCashNetworkcoinAddress addr(strAddress);
    if (!addr.IsValid())
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress \"StarCashNetworkaddress\" ( minconf )\n"
            "\nReturns the total amount received by the given StarCashNetworkaddress in transactions with at least minconf confirmations.\n"
            "\nArguments:\n"
            "1. \"StarCashNetworkaddress\"  (string, required) The StarCashNetwork address for transactions.\n"
            "2. minconf             (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"
            "\nResult:\n"
            "amount   (numeric) The total amount in STARS received at this address.\n"
            "\nExamples:\n"
    ...
    // Bitcoin address
    CStarCashNetworkcoinAddress address = CStarCashNetworkcoinAddress(params[0].get_str());
    CScript scriptPubKey;
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid StarCashNetwork address");
...
void GetSTARSuntAddresses(string strSTARSunt, set<CTxDestination>& setAddress)
{
...
Value getreceivedbyaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "2. minconf          (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"
            "\nResult:\n"
            "amount              (numeric) The total amount in STARS received for this account.\n"
            "\nExamples:\n"
    ...
    GetSTARSuntAddresses(strSTARSunt, setAddress);
...
int64_t GetSTARSuntBalance(CWalletDB& walletdb, const string& strSTARSunt, int nMinDepth, const isminefilter& filter)
{
    int64_t nBalance = 0;
    ...
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        ...
        wtx.GetSTARSuntAmounts(strSTARSunt, nReceived, nSent, nFee, filter);
    ...
    nBalance += walletdb.GetSTARSuntCreditDebit(strSTARSunt);

    return nBalance;
}

int64_t GetSTARSuntBalance(const string& strSTARSunt, int nMinDepth, const isminefilter& filter)
{
    CWalletDB walletdb(pwalletMain->strWalletFile);
    return GetSTARSuntBalance(walletdb, strSTARSunt, nMinDepth, filter);
}
...
Value getbalance(const Array& params, bool fHelp)
{
    ...
    if (params[0].get_str() == "*") {
        ...
        for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
        {
            ...
            CAmount allFee;
            string strSentSTARSunt;
            list<pair<CTxDestination, int64_t> > listReceived;
            list<pair<CTxDestination, int64_t> > listSent;
            wtx.GetAmounts(listReceived, listSent, allFee, strSentSTARSunt, filter);
            if (wtx.GetDepthInMainChain() >= nMinDepth)
            {
    ...
    CAmount nBalance = GetSTARSuntBalance(strSTARSunt, nMinDepth, filter);
...
Value movecmd(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 5)
        throw runtime_error(
            ...
            "\nExamples:\n"
            "\nMove 0.01 STARS from the default account to the account named tabby\n"
            + HelpExampleCli("move", "\"\" \"tabby\" 0.01") +
            "\nMove 0.01 STARS timotei to akiko with a comment and funds have 10 confirmations\n"
            + HelpExampleCli("move", "\"timotei\" \"akiko\" 0.01 10 \"happy birthday!\"") +
            "\nAs a json STARS call\n"
            + HelpExampleRpc("move", "\"timotei\", \"akiko\", 0.01, 10, \"happy birthday!\"")
        );

    accountingDeprecationCheck();

    string strFrom = STARSuntFromValue(params[0]);
    string strTo = STARSuntFromValue(params[1]);
    ...
    // Debit
    CSTARSuntingEntry debit;
    debit.nOrderPos = pwalletMain->IncOrderPosNext(&walletdb);
    debit.strSTARSunt = strFrom;
    debit.nCreditDebit = -nAmount;
    debit.nTime = nNow;
    debit.strOtherSTARSunt = strTo;
    debit.strComment = strComment;
    pwalletMain->AddSTARSuntingEntry(debit, walletdb);

    // Credit
    CSTARSuntingEntry credit;
    credit.nOrderPos = pwalletMain->IncOrderPosNext(&walletdb);
    credit.strSTARSunt = strTo;
    credit.nCreditDebit = nAmount;
    credit.nTime = nNow;
    credit.strOtherSTARSunt = strFrom;
    credit.strComment = strComment;
    pwalletMain->AddSTARSuntingEntry(credit, walletdb);
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 7)
        throw runtime_error(
            "sendfrom \"fromaccount\" \"toStarCashNetworkaddress\" amount ( minconf \"comment\" \"comment-to\" )\n"
            "\nSent an amount from an account to a StarCashNetwork address.\n"
            "The amount is a real and is rounded to the nearest 0.00000001."
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"fromaccount\"       (string, required) The name of the account to send funds from. May be the default account using \"\".\n"
            "2. \"toStarCashNetworkaddress\"  (string, required) The StarCashNetwork address to send funds to.\n"
            "3. amount                (numeric, required) The amount in STARS. (transaction fee is added on top).\n"
            ...
            "\"transactionid\"        (string) The transaction id.\n"
            "\nExamples:\n"
            "\nSend 0.01 STARS from the default account to the address, must have at least 1 confirmation\n"
    ...
    string strSTARSunt = STARSuntFromValue(params[0]);
    CStarCashNetworkcoinAddress address(params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid StarCashNetwork address");
    CAmount nAmount = AmountFromValue(params[2]);
    ...
    CWalletTx wtx;
    wtx.strFromSTARSunt = strSTARSunt;
    ...
    // Check funds
    int64_t nBalance = GetSTARSuntBalance(strSTARSunt, nMinDepth, ISMINE_SPENDABLE);
    if (nAmount > nBalance)
        throw JSONRPCError(RPC_WALLET_INSUFFICIENT_FUNDS, "STARSunt has insufficient funds");
...
Value sendmany(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric) The StarCashNetwork address is the key, the numeric amount in STARS is the value\n"
            "      ,...\n"
            "    }\n"
    ...
    set<CStarCashNetworkcoinAddress> setAddress;
    vector<pair<CScript, int64_t> > vecSend;

    int64_t totalAmount = 0;
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        CStarCashNetworkcoinAddress address(s.name_);
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid StarCashNetwork address: ")+s.name_);
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" )\n"
            ...
            "Each key is a StarCashNetwork address or hex-encoded public key.\n"
            "If 'account' is specified, assign address to that account.\n"

            "\nArguments:\n"
            "1. nrequired        (numeric, required) The number of required signatures out of the n keys or addresses.\n"
            "2. \"keysobject\"   (string, required) A json array of StarCashNetwork addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) StarCashNetwork address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
            "3. \"account\"      (string, optional) An account to assign the addresses to.\n"

            "\nResult:\n"
            "\"StarCashNetworkaddress\"  (string) A StarCashNetwork address associated with the keys.\n"
    ...
    string strSTARSunt;
    if (params.size() > 2)
        strSTARSunt = STARSuntFromValue(params[2]);
    ...
    pwalletMain->SetAddressBookName(innerID, strSTARSunt);
    return CStarCashNetworkcoinAddress(innerID).ToString();
}
...
Value ListReceived(const Array& params, bool fBySTARSunts)
{
    // Minimum confirmations
    int nMinDepth = 1;
    if (params.size() > 0)
    ...
    // Tally
    map<CStarCashNetworkcoinAddress, tallyitem> mapTally;
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        const CWalletTx& wtx = (*it).second;
    ...
    // Reply
    Array ret;
    map<string, tallyitem> mapSTARSuntTally;
    BOOST_FOREACH(const PAIRTYPE(CStarCashNetworkcoinAddress, string)& item, pwalletMain->mapAddressBook)
    {
        const CStarCashNetworkcoinAddress& address = item.first;
        const string& strSTARSunt = item.second;
        map<CStarCashNetworkcoinAddress, tallyitem>::iterator it = mapTally.find(address);
        ...
        if (fBySTARSunts)
        {
            tallyitem& item = mapSTARSuntTally[strSTARSunt];
        ...
        else
        {
            ...
            obj.push_back(Pair("account",       strSTARSunt));
    ...
    if (fBySTARSunts)
    {
        for (map<string, tallyitem>::iterator it = mapSTARSuntTally.begin(); it != mapSTARSuntTally.end(); ++it)
        {
...
Value listreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "    \"amount\" : x.xxx,                  (numeric) The total amount in STARS received by the address\n"
...
{
    CStarCashNetworkcoinAddress addr;
    if (addr.Set(dest))
        entry.push_back(Pair("address", addr.ToString()));
}

void ListTransactions(const CWalletTx& wtx, const string& strSTARSunt, int nMinDepth, bool fLong, Array& ret, const isminefilter& filter)
{
    CAmount nFee;
    string strSentSTARSunt;
    list<pair<CTxDestination, int64_t> > listReceived;
    ...
    wtx.GetAmounts(listReceived, listSent, nFee, strSentSTARSunt, filter);

    bool fAllSTARSunts = (strSTARSunt == string("*"));
    bool involvesWatchonly = wtx.IsFromMe(ISMINE_WATCH_ONLY);

    // Sent
    if ((!wtx.IsCoinStake()) && (!listSent.empty() || nFee != 0) && (fAllSTARSunts || strSTARSunt == strSentSTARSunt))
    {
        BOOST_FOREACH(const PAIRTYPE(CTxDestination, int64_t)& s, listSent)
        {
            ...
            entry.push_back(Pair("account", strSentSTARSunt));
    ...
    if (listReceived.size() > 0 && wtx.GetDepthInMainChain() >= nMinDepth)
    {
        ...
        BOOST_FOREACH(const PAIRTYPE(CTxDestination, int64_t)& r, listReceived)
        {
            ...
            if (fAllSTARSunts || (account == strSTARSunt))
            {
                Object entry;
...
void AcentryToJSON(const CSTARSuntingEntry& acentry, const string& strSTARSunt, Array& ret)
{
    bool fAllSTARSunts = (strSTARSunt == string("*"));

    if (fAllSTARSunts || acentry.strSTARSunt == strSTARSunt)
    {
        Object entry;
        entry.push_back(Pair("account", acentry.strSTARSunt));
        entry.push_back(Pair("category", "move"));
        entry.push_back(Pair("time", (int64_t)acentry.nTime));
        entry.push_back(Pair("amount", ValueFromAmount(acentry.nCreditDebit)));
        entry.push_back(Pair("otheraccount", acentry.strOtherSTARSunt));
...
Value listtransactions(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 4)
        throw runtime_error(
            ...
            "    \"address\":\"StarCashNetworkaddress\",    (string) The StarCashNetwork address of the transaction. Not present for \n"
            "                                                move transactions (category = move).\n"
            "    \"category\":\"send|receive|move\", (string) The transaction category. 'move' is a local (off blockchain)\n"
            "                                                transaction between accounts, and not associated with an address,\n"
            "                                                transaction id or block. 'send' and 'receive' transactions are \n"
            "                                                associated with an address, transaction id and block details\n"
            "    \"amount\": x.xxx,          (numeric) The amount in STARS. This is negative for the 'send' category, and for the\n"
    ...
    for (CWallet::TxItems::const_reverse_iterator it = txOrdered.rbegin(); it != txOrdered.rend(); ++it)
    {
        ...
        if (pwtx != 0)
            ListTransactions(*pwtx, strSTARSunt, 0, true, ret, filter);
        CSTARSuntingEntry *const pacentry = (*it).second.second;
        if (pacentry != 0)
            AcentryToJSON(*pacentry, strSTARSunt, ret);
...
Value listaccounts(const Array& params, bool fHelp)
{
    ...
    map<string, CAmount> mapSTARSuntBalances;
    BOOST_FOREACH(const PAIRTYPE(CTxDestination, string)& entry, pwalletMain->mapAddressBook) {
        if (IsMine(*pwalletMain, entry.first) & includeWatchonly) // This address belongs to me
            mapSTARSuntBalances[entry.second] = 0;
    }
    ...
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        ...
        if (nDepth < 0)
            continue;
        wtx.GetAmounts(listReceived, listSent, nFee, strSentSTARSunt, includeWatchonly);
        mapSTARSuntBalances[strSentSTARSunt] -= nFee;
        BOOST_FOREACH(const PAIRTYPE(CTxDestination, int64_t)& s, listSent)
            mapSTARSuntBalances[strSentSTARSunt] -= s.second;
        if (nDepth >= nMinDepth && wtx.GetBlocksToMaturity() == 0)
        {
            BOOST_FOREACH(const PAIRTYPE(CTxDestination, int64_t)& r, listReceived)
                if (pwalletMain->mapAddressBook.count(r.first))
                    mapSTARSuntBalances[pwalletMain->mapAddressBook[r.first]] += r.second;
                else
                    mapSTARSuntBalances[""] += r.second;
        }
    }

    const list<CSTARSuntingEntry> & acentries = pwalletMain->laccentries;
    BOOST_FOREACH(const CSTARSuntingEntry& entry, acentries)
        mapSTARSuntBalances[entry.strSTARSunt] += entry.nCreditDebit;

    Object ret;
    BOOST_FOREACH(const PAIRTYPE(string, CAmount)& accountBalance, mapSTARSuntBalances) {
...
Value listsinceblock(const Array& params, bool fHelp)
{
    if (fHelp)
       throw runtime_error(
            ...
            "    \"address\":\"StarCashNetworkaddress\",    (string) The StarCashNetwork address of the transaction. Not present for move transactions (category = move).\n"
            "    \"category\":\"send|receive\",     (string) The transaction category. 'send' has negative amounts, 'receive' has positive amounts.\n"
            "    \"amount\": x.xxx,          (numeric) The amount in STARS. This is negative for the 'send' category, and for the 'move' category for moves \n"
...
Value gettransaction(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "      \"address\" : \"StarCashNetworkaddress\",   (string) The StarCashNetwork address involved in the transaction\n"
...
Value walletpassphrase(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 3))
        throw runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending STARS\n"
            "\nArguments:\n"
...
Value encryptwallet(const Array& params, bool fHelp)
{
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending StarCashNetwork\n"
            + HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
            "\nNow we can so something like sign\n"
            + HelpExampleCli("signmessage", "\"StarCashNetworkaddress\" \"test message\"") +
    ...
    StartShutdown();
    return "wallet encrypted; StarCashNetwork server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.";
}
...
Value settxfee(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. amount         (numeric, required) The transaction fee in STARS/kB rounded to the nearest 0.00000001\n"
            "\nResult\n"
...
Value getnewstealthaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new StarCashNetwork stealth address for receiving payments anonymously.  ");
...
Value sendtostealthaddress(const Array& params, bool fHelp)
{
    ...
    {
        result.push_back(Pair("result", "Invalid StarCashNetwork stealth address."));
        return result;
    };
