...
static void
GetResults(CWalletDB& walletdb, std::map<int64, CSTARSuntingEntry>& results)
{
    std::list<CSTARSuntingEntry> aes;

    results.clear();
    BOOST_CHECK(walletdb.ReorderTransactions(pwalletMain) == DB_LOAD_OK);
    walletdb.ListSTARSuntCreditDebit("", aes);
    BOOST_FOREACH(CSTARSuntingEntry& ae, aes)
    {
...
BOOST_AUTO_TEST_CASE(acc_orderupgrade)
{
    ...
    CWalletTx wtx;
    CSTARSuntingEntry ae;
    std::map<int64, CSTARSuntingEntry> results;

    ae.strSTARSunt = "";
    ae.nCreditDebit = 1;
    ae.nTime = 1333333333;
    ae.strOtherSTARSunt = "b";
    ae.strComment = "";
    walletdb.WriteSTARSuntingEntry(ae);
    ...
    BOOST_CHECK(results[2].strOtherSTARSunt == "c");


    ae.nTime = 1333333330;
    ae.strOtherSTARSunt = "d";
