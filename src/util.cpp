...
int nAnonymizeStarCashNetworkAmount = 1000;
...
#else
    const char* pszModule = "StarCashNetwork";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\StarCashNetwork
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\StarCashNetwork
    // Mac: ~/Library/Application Support/StarCashNetwork
    // Unix: ~/.StarCashNetwork
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "StarCashNetwork";
#else
    fs::path pathRet;
    ...
    return pathRet / "StarCashNetwork";
#else
    // Unix
    return pathRet / ".StarCashNetwork";
#endif
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "StarCashNetwork.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "StarCashNetworkd.pid"));
