...
public:
    ...
    std::list<CSTARSuntingEntry> laccentries;

    typedef std::pair<CWalletTx*, CSTARSuntingEntry*> TxPair;
    ...
    bool AddSTARSuntingEntry(const CSTARSuntingEntry&, CWalletDB & pwalletdb);
...
public:
    ...
    (
        ...
        if (!fRead)
        {
            pthis->mapValue["fromaccount"] = pthis->strFromSTARSunt;
        ...
        if (fRead)
        {
            pthis->strFromSTARSunt = pthis->mapValue["fromaccount"];
    ...
    void GetAmounts(std::list<std::pair<CTxDestination, int64_t> >& listReceived,
                    std::list<std::pair<CTxDestination, int64_t> >& listSent, CAmount& nFee, std::string& strSentSTARSunt, const isminefilter& filter) const;

    void GetSTARSuntAmounts(const std::string& strSTARSunt, CAmount& nReceived,
...
{
public:
    std::string strSTARSunt;
    int64_t nCreditDebit;
    int64_t nTime;
    std::string strOtherSTARSunt;
    std::string strComment;
    mapValue_t mapValue;
    int64_t nOrderPos;  // position in ordered transaction list
    uint64_t nEntryNo;
    ...
    IMPLEMENT_SERIALIZE
    (
        CSTARSuntingEntry& me = *const_cast<CSTARSuntingEntry*>(this);
        if (!(nType & SER_GETHASH))
            READWRITE(nVersion);
        // Note: strSTARSunt is serialized as part of the key, not here.
        READWRITE(nCreditDebit);
        READWRITE(nTime);
        READWRITE(strOtherSTARSunt);
